...
def main():
    ...
    parser.add_argument("--weights", default="culane_18.pth", help="Path to the CULane pretrained weights")
    parser.add_argument("--device", default="cpu", help="cpu or cuda")
    args = parser.parse_args()

    os.makedirs(args.output, exist_ok=True)
    model = detector.load_model(args.weights, args.device)
    ...
    if images:
        ...
        if warmup is not None:
            detector.detect(model, warmup, args.device)
